...
window.toggleReviews = function() {
  ...
  if (span.text() === 'show') {
    ...
    $.ajax({
      ...
      data: JSON.stringify({
        text: reviewsText,
        created_at: reviewsCreated_at,
        updated: reviewsUpdated
      }),
      success: function(data) {
        // Update reviewsList with the parsed reviews data
        for (var i = 0; i < data.length; i++) {
          var review = data[i];
          var article = '<article>' +
            ...
            '<div class="reviews">' +
              ...
              '<small class="review_date">' + review.created_at + '</small>' +
              '<small class="review_updated">' + review.updated + ' Bathroom' +  '</small>' +
            '</div>' +
